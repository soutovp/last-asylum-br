...
export default function HeroDetail({ hero }: HeroDetailProps) {
  const [selectedStar, setSelectedStar] = useState<string>("2★");
  const starOptions = ["1★", "2★", "4★", "6★ (🔴1)", "8★ (🔴3)", "10★ (🔴5)"];

  const relatedHeroes = getRelatedHeroes(hero, 3);
  const factionLabel = getFactionLabel(hero.faction);
  const roleLabel = getRoleLabel(hero.role);
  const damageTypeLabel = getDamageTypeLabel(hero.damageType);
  const isDay1 = hero.unlockInfo.serverDay === 1;

  const getSkillEffectForStar = (skill: Hero["skills"][0], star: string): string => {
    if (star === "1★") return skill.description;
    ...
    const targetStars = starMap[star];
    if (!targetStars || !skill.progression) return skill.description;

    const match = skill.progression.find((p) => p.starsRequired === targetStars);
    return match ? match.description : skill.description;
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-12">
      ...
      <nav aria-label="Breadcrumb" className="text-sm text-slate-400 flex items-center gap-2">
        ...
        <span>/</span>
        <span className="text-slate-200 font-semibold">{hero.name}</span>
      </nav>
      ...
      <div className="bg-[#101623]/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-md flex flex-col md:flex-row gap-8 items-center md:items-start relative overflow-hidden">
        ...
        <div className="w-48 h-48 sm:w-56 sm:h-56 bg-slate-900 rounded-2xl flex-shrink-0 border-2 border-slate-700 relative overflow-hidden flex items-center justify-center shadow-lg">
          {hero.avatarUrl ? (
            ...
            <span className="text-5xl text-slate-700 font-bold">{hero.name[0]}</span>
          )}
          <div className="absolute top-3 left-3">
            <span className={`px-3 py-1 rounded-md text-xs font-black border ${getRarityBadgeColor(hero.rarity)}`}>
              {hero.rarity}
            </span>
          </div>
          {hero.tier && (
        ...
        <div className="flex-1 space-y-4 text-center md:text-left">
          <div className="flex flex-wrap gap-2 justify-center md:justify-start">
            <span className={`px-3 py-1 rounded-md text-xs font-semibold border ${getFactionBadgeColor(hero.faction)}`}>
              {factionLabel} ({hero.faction})
            </span>
            <span className={`px-3 py-1 rounded-md text-xs font-semibold border ${getRoleBadgeColor(hero.role)}`}>
              {roleLabel}
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#080c14] p-5 rounded-xl border border-slate-800/70 space-y-2">
            ...
            <p className="text-xl font-bold text-white">
              {isDay1 ? (
                ...
                <span className="text-amber-400">Desbloqueia no Dia {hero.unlockInfo.serverDay} do Servidor</span>
              )}
            </p>
            <p className="text-xs text-slate-400 leading-relaxed">
              {hero.unlockInfo.notes}
            </p>
          </div>
          ...
          <div className="bg-[#080c14] p-5 rounded-xl border border-slate-800/70 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {hero.unlockInfo.methods.map((method, idx) => (
                <span
                  key={idx}
              ...
              {hero.unlockInfo.laterMethods?.map((later, idx) => (
                <span
                  key={`later-${idx}`}
      ...
      <section className="bg-[#101623]/80 border border-slate-800 rounded-2xl p-6 sm:p-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-[#080c14] p-4 rounded-xl border border-slate-800 text-center">
            ...
            <span className="text-sm sm:text-base font-bold text-white">{getPositionLabel(hero.combatProfile.position)}</span>
          </div>
          <div className="bg-[#080c14] p-4 rounded-xl border border-slate-800 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
          <div className="bg-emerald-950/20 border border-emerald-900/40 p-5 rounded-xl space-y-3">
            ...
            <ul className="space-y-2 text-xs sm:text-sm text-slate-200">
              {hero.combatProfile.pros.map((pro, idx) => (
          ...
          <div className="bg-red-950/20 border border-red-900/40 p-5 rounded-xl space-y-3">
            ...
            <ul className="space-y-2 text-xs sm:text-sm text-slate-200">
              {hero.combatProfile.cons.map((con, idx) => (
                <li key={idx} className="flex items-start gap-2">
        ...
        <div className="bg-[#080c14] p-5 rounded-xl border border-slate-800 space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {hero.combatProfile.tags.map((tag) => (
                <span
                  key={tag}
          ...
          <div className="pt-2 border-t border-slate-800">
            ...
            <p className="text-sm text-slate-200">{hero.combatProfile.bestFormation}</p>
            {hero.combatProfile.recommendedPositioningNote && (
              <p className="text-xs text-slate-400 mt-1 italic">
                Dica tática: {hero.combatProfile.recommendedPositioningNote}
              </p>
            )}
          </div>
        </div>
      ...
      <section className="bg-[#101623]/80 border border-slate-800 rounded-2xl p-6 sm:p-8 space-y-8">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-2">
            Selecione o nível de estrelas para simular o efeito exato de cada habilidade de {hero.name} em combate.
          </p>
        </div>
        ...
        <div className="space-y-6">
          {hero.skills.map((skill, idx) => {
            const starDescription = getSkillEffectForStar(skill, selectedStar);
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    ...
                    {skill.energyCost && (
                      <span className="bg-slate-900 border border-slate-700 px-2.5 py-1 rounded text-cyan-300">
                ...
                {selectedStar !== "1★" && (
                  <div className="bg-[#101623] p-4 rounded-xl border border-amber-500/30 space-y-1">
                    <span className="text-xs text-amber-400 font-semibold block">
                      Aprimoramento no Nível {selectedStar}:
                    </span>
      ...
      <section className="bg-gradient-to-br from-slate-900 via-[#101623] to-[#080c14] border border-[#00ff88]/30 rounded-3xl p-6 sm:p-8 relative overflow-hidden space-y-6 shadow-xl">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-3">
            <span className="w-2.5 h-6 bg-[#00ff88] rounded-full inline-block"></span>
            Calculadoras de Recursos para {hero.name}
          </h2>
          <p className="text-sm text-slate-300 mt-2 max-w-2xl">
            Planeje com exatidão a quantidade de Fragmentos, Antitoxina e Insígnias de Habilidade necessárias para evoluir {hero.name} até o nível máximo.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          <Link
            href={hero.calculatorLinks?.shardsUrl || `/calculadoras?tab=shards&hero=${hero.slug}`}
          ...
          >
            ...
            <span className="text-xs font-semibold mt-4 text-slate-900 flex items-center gap-1">
              Calcular Shards de {hero.name} →
            </span>
          </Link>

          <Link
            href={hero.calculatorLinks?.antitoxinUrl || `/calculadoras?tab=antitoxin&hero=${hero.slug}`}
          ...
          <Link
            href={hero.calculatorLinks?.badgesUrl || `/calculadoras?tab=badges&hero=${hero.slug}`}
            className="bg-slate-900/90 hover:bg-slate-800 text-white border border-slate-700 hover:border-[#00ff88] font-bold p-5 rounded-2xl transition-all flex flex-col justify-between"
          >
            <div>
              <span className="text-xs uppercase tracking-wider text-slate-400 block mb-1">
      ...
      {/* 6. SEÇÃO DE BIOGRAFIA E HISTÓRIA */}
      {hero.lore && (
        <section className="bg-[#101623]/80 border border-slate-800 rounded-2xl p-6 sm:p-8 space-y-4">
          <h2 className="text-2xl font-bold text-white border-b border-slate-800 pb-3 flex items-center gap-3">
            <span className="w-2.5 h-6 bg-slate-500 rounded-full inline-block"></span>
            História e Origem de {hero.name}
          </h2>
          <p className="text-sm text-slate-300 leading-relaxed">{hero.lore}</p>
        </section>
      )}
    </div>
  );
